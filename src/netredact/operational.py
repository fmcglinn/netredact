...
from .rules import FortiBlocks, routeros_scope
...
class OperationalNames:
    ...
    def __init__(self, policy: OperationalNamesPolicy, pseudonymiser):
        ...
        #: the FortiOS block stack, tracked here for the same reason, and it is
        #: the catalogue's own class so the three passes cannot disagree about
        #: where a block ends. An `edit` name is an interface under `config
        #: system interface` and a policy id under `config firewall policy`,
        #: and only the block says which.
        self._forti = FortiBlocks()
    ...
    def line(self, line: str) -> str:
        ...
        # scopes BEFORE feed, so a `config` line is not inside the block it
        # opens -- the convention `FortiBlocks` documents and the collect pass
        # already follows
        fos = self._forti.scopes()
        self._forti.feed(line)
