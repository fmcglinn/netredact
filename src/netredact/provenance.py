...
def _comment(vendor: str) -> str:
    ...
    JunOS, RouterOS and FortiOS comment with ``#`` -- FortiOS writes its own
    header that way -- and IOS-style grammars use ``!``. An unknown vendor
    gets ``!``, which the ``set`` format also tolerates at the start of a
    line.
    """
    return "#" if vendor in ("juniper", "mikrotik", "fortinet") else "!"
