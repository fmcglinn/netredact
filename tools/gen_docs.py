...
RULE_NOTES = {
    ...
    "bare-password": "an indented `password` / `passwd` line, e.g. under "
                     "`line vty`, and FortiOS `set password ENC X`",
    "bare-secret": "a bare `secret` line, including JunOS `set ... secret` "
                   "and FortiOS `set secret` / `set psksecret`",
    "fortios-encrypted": "FortiOS `set <key> ENC X` -- the marker FortiOS "
                         "writes in front of a stored credential and in front "
                         "of nothing else, so the rule needs no list of keys",
    "fortios-credential-key": "FortiOS `set <qualifier>-password|passwd|pwd|"
                              "secret|passphrase X` without the `ENC` marker -- "
                              "the cleartext a typed or templated configuration "
                              "carries",
    "fortios-snmp-community": "`set name` under `config system snmp community`, "
                              "which is what FortiOS calls a community string",
